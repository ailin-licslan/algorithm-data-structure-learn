...
public class SelectionSort {
    ...
    public static ArrayList sortAfter(int[] sortBefore,ArrayList list){
        //3,4,1,5  先把最小拿出来  剩下的，再把其中最小的拿出来 ......
        //每次选择还没处理的元素里最小的元素
        //定义最小值为该数组的第一个数
        int minIndex = sortBefore[0];
        ...
        for (int i = 0; i < sortBefore.length; i++) {
            //The index need to be deleted
            if(minIndex > sortBefore[i]){minIndex = sortBefore[i]; indexDel=i;}
        }
        list.add(minIndex);
        //删除指定元素的生成新的数组
        int[] intNew = delAnyPos(sortBefore, indexDel);
        if(Objects.requireNonNull(intNew).length>0) {
    ...
    public static void sort(int[] arr){
        ...
        for (int i = 0; i < arr.length; i++) {
            //arr[i...n)中最小的值索引
            //arr[i...n) 目前认为这个位置最小 arr[i] （其实不一定是最小）
            int minIndex = i;
